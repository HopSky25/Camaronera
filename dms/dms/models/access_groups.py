...
class DmsAccessGroups(models.Model):
    ...
    @api.depends(
        ...
        "group_ids",
        "group_ids.all_user_ids",
        "explicit_user_ids",
    )
    def _compute_users(self):
        # res.groups.users was split in 19.0: user_ids only holds explicit
        # members, while all_user_ids also resolves implied groups. Until 18.0
        # the relation table was denormalized and already contained implied
        # members, so all_user_ids is the equivalent -- using user_ids here
        # would silently drop access for anyone holding an implying group.
        for record in self:
            users = (
                record.group_ids.all_user_ids
