...
class DmsSecurityMixin(models.AbstractModel):
    ...
    def _get_domain_by_inheritance(self, operation):
        ...
        related_groups = self.sudo()._read_group(
            inherited_access_domain & Domain("res_model", "!=", False),
            groupby=["res_model"],
            aggregates=["res_id:array_agg"],
        )
        for res_model, res_id_agg in related_groups:
            try:
            ...
            try:
                ...
                continue
            domains.append(
                Domain("res_model", "=", model._name) & Domain("res_id", "=", False)
            )
            # Check record access in batch too
            res_ids = [i for i in res_id_agg if i]  # Hack to remove None res_id
            ...
            domains.append(
                ...
                & Domain("res_id", "in", related_ok.ids)
            )
    ...
    def _get_permission_domain(self, operator, value, operation):
        ...
        if operator != "in":
            # 19.0 normalizes conditions on a boolean field to `in [True]` or
            # `not in [True]`. Returning NotImplemented makes the ORM ask for
            # the positive domain and negate it itself.
            return NotImplemented
        # HACK ir.rule domains are always optimized under sudo, so env.su is on
        # even while the rule is being applied to a regular user. Until 18.0 the
        # rules smuggled `user.id` in as the compared value so this method could
        # tell the two cases apart, but 19.0 casts the operand of a boolean
        # condition to True and the marker is lost. The effective uid survives
        # sudo(), so key on that instead: only genuine superuser code gets the
        # unrestricted domain.
    ...
    def _can_return_content(self, field_name=None, access_token=None):
        """Serve binary content to anyone holding a valid DMS share token.

        Replaces the 18.0 override of ``ir.binary._find_record_check_access()``,
        which no longer exists in 19.0. The caller
        (``ir.binary._find_record()``) applies the ``sudo()`` itself when this
        returns True, so the user does not need read access to the record.
        Used, among others, to render icons and previews in the portal.
        """
        self.ensure_one()
